...
def data2vonMises( angles, values, n_clust=1, im_name=None ):
    """
    Provide the angles and values (intensity) from the FFT of image
    'im_name', and the number of clusters n_clust
    ...
    im_name : string, the name of the image file from which the fibers 
...
#    # ============== 
    ...
    res_vonMF = saveFitMixvMises( vmf_soft, im_name )
    
    return res_vonMF
        

def normalizeIntensity( angles, values, YNplot='Yes' ):
    """Function to normalize the intensity data.
    ...
    # number of equidistant intervals of (-pi/2,pi/2)
...
def makePoints( n_X, YNplot='Yes' ):
    ...
    """
    # the equidistant increments of the space [-pi/2,pi/2]:
    bin_angle, _ = np.shape(n_X)
    # population factor:
    sfact = 0.55
    # convert the intensity to population per angle:
    ...
    if YNplot == 'Yes':
        ...
        axes[0].legend()
        axes[1].hist( p_Xd, bins=bin_angle, label='degrees', alpha=0.5 );
...
def saveFitMixvMises( mixvM, im_name ):
    ...
    # save the .txt file:
    np.savetxt( arr_name, res_vonMF, fmt='%.18e', delimiter=' ', \
...
def plotMixvMises1X2( res_vonMF, im_name, n_X ):
    """
    ...
    im_name   : string, the name of the image file
    ...
    fig_name_eps = fig_name + '.eps'
    # save the .png file:
    fig.savefig(fig_name)
    fig.savefig(fig_name_eps)
...
def test():
    ...
    csv_file = "MAX_noepi_C2.csv"
    n_clust = 2
    angles, values = image2Data( csv_file )
    res_vonMF = data2vonMises( angles, values, n_clust, im_name )
    YNplot = 'Yes'
    if YNplot == 'Yes':
        n_X = normalizeIntensity( angles, values )
        plotMixvMises1X2( res_vonMF, im_name, n_X )
...
def myPPplot( cfX_tot, cfX_exp, im_name, n_clus ):
    '''
